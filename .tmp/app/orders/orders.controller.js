...
angular.module('buildoutletApp').controller('OrdersCtrl', ['$scope', 'Order', 'Auth', 'Modal', function ($scope, Order, Auth, Modal) {
  var user = Auth.getCurrentUser();
  $scope.isAdmin = Auth.hasRole('admin');

  if ($scope.isAdmin) Order.query(function (orders) {
    $scope.orders = orders;
    // pagination controls
    $scope.currentPage = 1;
  ...
  });else Order.myOrders({ id: user._id }, function (orders) {
    $scope.orders = orders;
    // pagination controls
    $scope.currentPage = 1;
    $scope.totalItems = $scope.orders.length;
