...
angular.module('buildoutletApp').controller('ManageRequestsCtrl', ['$scope', 'Modal', 'Request', 'Auth', function ($scope, Modal, Request, Auth) {
  var user = Auth.getCurrentUser();
  $scope.isAdmin = Auth.hasRole('admin');
  $scope.requests = [];

  if ($scope.isAdmin) Request.query(function (requests) {
  ...
  });else Request.myRequests({ id: user._id }, function (requests) {
    $scope.requests = requests;
    // pagination controls
    $scope.currentPage = 1;
    $scope.totalItems = $scope.requests.length;
...
}]).controller('SubmitRequestCtrl', ['$scope', 'Modal', 'Request', 'Auth', '$state', function ($scope, Modal, Request, Auth, $state) {
  ...
  $scope.addToList = function (newItem) {
    ...
    $scope.listSubTotal = _.reduce($scope.items, function (a, b) {
      return a + b.total;
    }, 0);
  ...
  $scope.submitList = function (items, user) {
    if (user.name && user.email && user.phone && user.address && user.city && user.country) {
      ...
      Request.save(list, function (resp) {
        console.log(resp);
